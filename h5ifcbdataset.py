...
class H5IFCBDataset(Dataset):
    def __init__(
        ...
        for i in tqdm(range(len(files)), desc="Loading samples: ", disable=(self.verbose < 1)):
            ...
            f.close()
        if metadata_file is not None:
            self.metadata = self.metadata.loc[sample_names]
        ...
        if trainingset and files:
            ...
            if missing_classes.size > 0:
                print(
                    "ERROR. YOU MUST REBUILD THE DATASET! Not all the classes are in the training set: {}. Remove them from the classes list and start again.".format(
                        missing_classes
                    )
                )
